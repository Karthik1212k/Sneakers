...
export default function ProductGrid() {
  ...
  const filtered = useMemo(() => {
    let base = active === "All" ? products : products.filter((p) => p.brand === active);
    ...
    return base.map((p, idx) => (idx >= start ? { ...p, image: fallbackLastFour[idx - start] } : p));
  }, [active, q]);
    ...
    <section id="featured" className="container mx-auto px-4 py-12 sm:py-16">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h2 className="heading-display mt-2 text-2xl font-semibold sm:text-3xl">Latest Drops</h2>
        </div>
          ...
          {brands.map((b) => (
            <button
              ...
              className={`h-9 rounded-md border px-3 text-sm font-medium transition-colors ${
                active === b ? "border-primary bg-primary/10 text-primary" : "hover:bg-muted"
              }`}
            >
              {b}
            </button>
      ...
      {q && (
        <p className="mb-4 text-sm text-muted-foreground">Showing results for "{q}"</p>
      )}
        ...
        {filtered
          ...
          .map((p) => (
            <ProductCard
              key={p.id}
              product={p}
              onAdd={(prod) => addItem({ id: prod.id, name: prod.name, price: prod.price, image: prod.image, brand: prod.brand })}
            />
          ))}
      </div>
    </section>
